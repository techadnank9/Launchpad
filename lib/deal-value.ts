/** Persona-specific deal economics inferred from the seller's site (e.g. thecorgi.cafe). */
...
/**
 * "Rate of motion" — how actively the lead's company is moving (hiring, news, buzz).
 * Drives position inside the persona's deal range.
 */
export function computeMotion(breakdown: {
  ...
  if (breakdown.jobCount > 0) {
    const boost = Math.min(38, 12 + breakdown.jobCount * 7);
    motion += boost;
    factors.push(
  ...
  if (breakdown.newsCount > 0) {
    const boost = Math.min(28, 8 + breakdown.newsCount * 5);
    motion += boost;
    factors.push(
...
/**
 * Deal value = position inside persona ACV range, set by motion (not a fake global formula).
 * Low motion → near min. High motion → near max.
 */
export function computeDealValue(
  ...
  const explanation = `${economics.pricingModel} · ${formatUsd(min)}–${formatUsd(max)} segment · ${motionLabel} (${motionScore}/100)`;
...
/** Sensible defaults when GPT omits economics (legacy personas). */
